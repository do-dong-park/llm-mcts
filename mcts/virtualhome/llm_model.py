...
Now, answer the following questions:\n
"""
        # container, grabable, surface object 자체, object position, room에 대한 embedding
        self.translation_lm = SentenceTransformer("stsb-roberta-large").to(self.device)
        self.object_info = json.load(open("vh/data_gene/dataset/object_info.json", "r"))
        ...
        self.container_list_embedding = self.translation_lm.encode(
            self.container_list,
            batch_size=8,
            convert_to_tensor=True,
            device=self.device,
        )  # lower batch_size if limited by GPU memory
        self.surface_list_embedding = self.translation_lm.encode(
            self.surface_list, batch_size=8, convert_to_tensor=True, device=self.device
        )
        self.grabable_list_embedding = self.translation_lm.encode(
            self.grabable_list, batch_size=8, convert_to_tensor=True, device=self.device
        )
        self.position_list = self.container_list + self.surface_list
        self.position_list_embedding = torch.concat(
            (self.container_list_embedding, self.surface_list_embedding), dim=0
        )
        self.room_embedding = self.translation_lm.encode(
        ...
        for i in range(5):
            if obs_list[i] < 5:
                if i == 0:
                    ...
                    obs_str += (
                        self.item_list[i] + " is " + self.state_list[obs_list[i]] + ", "
                    )
        return obs_str[:-2] + "."
    ...
    def init_translation_trf(self):
        ...
        self.action_list_embedding = self.translation_lm.encode(
            self.action_list, batch_size=8, convert_to_tensor=True, device=self.device
        )  # lower batch_size if limited by GPU memory
    ...
    def find_top_k_most_similar(self, query_embedding, corpus_embedding, k):
        ...
        cos_scores = (
            st_utils.pytorch_cos_sim(query_embedding, corpus_embedding)[0]
            .detach()
            .cpu()
            .numpy()
        )
    ...
    def find_most_similar(self, query_str, corpus_embedding):
        # helper function for finding similar sentence in a corpus given a query
        query_embedding = self.translation_lm.encode(
            query_str,
            convert_to_tensor=True,
            device=self.device,
    ...
    def plan(self, task):
        ...
        for sample in samples:
            most_similar_idx, matching_score = self.find_most_similar(
                sample, self.action_list_embedding
            )
        ...
        #     return
            ...
            formatted_action = (best_action[0].upper() + best_action[1:]).replace(
                "_", " "
    ...
    def ground_actions(
        self, task, instruction, observation, valid_action_embedding, valid_actions
    ):
        try_times = 0
        while 1:
            try:
    ...
    def find_similar_task_and_observation(self, query_task, curr_observation, k):
        ...
        # calculate cosine similarity against each candidate sentence in the corpus
        task_embedding = self.translation_lm.encode(
            query_task,
            convert_to_tensor=True,
            device=self.device,
            show_progress_bar=False,
        )
        most_similar_task_idxs, _ = self.find_top_k_most_similar(
            task_embedding, self.task_embedding, k
        )
        # retrieve high-ranked index and similarity score
        # find the most similar observation given the most similar task
        most_similar_obs_idxs = [
            self.find_most_similar(
                curr_observation,
                self.observation_embedding[most_similar_task_idx].to(self.device),
            )
            for most_similar_task_idx in most_similar_task_idxs
        ]
    ...
    def get_prompt_examples(self, task, observation, k):
        ...
        (
            most_similar_task_idxs,
            most_similar_obs_idxs,
        ) = self.find_similar_task_and_observation([task], observation, k)
        prompt_examples = [
            self.get_prompt(most_similar_task_idx, most_similar_obs_idx)
            for most_similar_task_idx, most_similar_obs_idx in zip(
                most_similar_task_idxs, most_similar_obs_idxs
            )
        ]
        prompt_examples = (
        ...
        for subgoal in subgoals:
            ...
            if relation == "INSIDE":
                if container in self.room_list:
                    formal_goal[
                        f"inside_{target_object}_{container_name2id[container]}"
                    ] = [num, True, 2]
                else:
                    formal_goal[
                        f"inside_{target_object}_{container_name2id[container]}"
                ...
                formal_goal[f"on_{target_object}_{container_name2id[container]}"] = [
                    num,
                    True,
                    2,
                ]
    ...
    def query_llm(self, task, ins, curr_obs, k=3):
        ...
        while 1:
            try:
                ...
                print(task)  # Question: What is the most possible position of alcohol?
                print(response.choices[0].message.content)
                print()
                break
    ...
    def construct_house_model(self):
        ...
        for object in self.grabable_list:
            # 집안 물건에 대한 위치 정보
            samples = self.query_llm(
                f"Question: What is the most possible position of {object}?\nAnswer: ",
                "",
                "",
                1,
            )
        ...
        # save samples
        json.dump(object_positions, open(f"{data_dir}/object_all_positions.json", "w"))
        print("obj all position 저장 완료")
    ...
    def calculate_emperical_prob(
        ...
        if len(history) > 0:
            ...
            hist_text = ""
        valid_actions_lang = get_action_list_valid(
            valid_action_list, len(valid_action_list)
        )
        # print(valid_actions_lang)
        valid_action_embedding = self.translation_lm.encode(
            valid_actions_lang,
            convert_to_tensor=True,
            show_progress_bar=False,
            device=self.device,
        ...
        # task = f"{instruction}\nCompleted actions: {', '.join(hist_text)}.\n{observation}\nNext plan:"
        samples, num_steps, is_done = self.ground_actions(
            task, instruction, observation, valid_action_embedding, valid_action_list
        )
        pred_value = self.trivial_pred_value_function(
            history, num_steps, is_done, done_reward, step_reward, discount_factor
        )
        action_count = {}
        ...
        for action in valid_action_list:
            if action in action_count:
                emperical_prob.append(
                    LAMBDA * action_count[action] / len(samples)
                    + (1 - LAMBDA) / len(valid_action_list)
                )
            else:
    ...
    def act(self, history, observation, valid_action_list, instruction):
        ...
        task = f"{instruction}\nCompleted actions: {', '.join(hist_text)}.\nNext plan:"
        # task = f"{instruction}\nCompleted actions: {', '.join(hist_text)}.\n{observation}\nNext plan:"
        samples, _, _ = self.ground_actions(
            task, instruction, observation, valid_action_embedding, valid_action_list
        )
        return random.choice(samples)

    def trivial_pred_value_function(
        self, history, num_of_steps, is_done, done_reward, step_reward, discount_factor
    ):
    ...
    def rollout(
        self, history, observation, valid_actions, instruction, discount_factor
    ):
        """return number of remaining steps"""
        valid_action_list = list(valid_actions.keys())
        valid_action_embedding = self.translation_lm.encode(
            valid_action_list, convert_to_tensor=True, device=self.device
        )
...
if __name__ == "__main__":
    llm_agent = LLM_Model("cpu")
    llm_agent.construct_house_model()
    llm_agent.post_process(
        f"{data_dir}/object_all_positions.json", f"{data_dir}/obj_commonsense.json"
    )
    llm_agent.post_process(
        f"{data_dir}/furniture_all_positions.json",
        f"{data_dir}/fur_commonsense.json",
    )
