...
from vh.learned_policy.data_loader import (
    get_goal_language,
    get_history_action_input_language,
    parse_language_from_action_script,
)
...
def translate_dataset(device):
    ...
    expert_files = [
        "./expert_actions/expert_full/" + file
    ...
    expert_files_2 = [
        "./expert_actions/expert_simple/" + file
        for file in expert_files_2
        if file.endswith(".pik")
    ]
    ...
    for file in tqdm(expert_files):
        ...
        act_langs = get_action_list_all(
            expert_data["action"], len(expert_data["action"][0])
        )
...
def get_action_list_all(acts, step_i, agent_id=0):
    ...
    if len(previous_acts) > 0:
        ...
        goal_actions_parsed = [
            parse_language_from_action_script(tem) for tem in goal_actions
        ]
        history_actions = []
