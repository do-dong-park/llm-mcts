...
from vh.vh_sim.simulation.evolving_graph.utils import (
    load_graph_dict,
...
class Belief:
    ...
    def init_belief_commonsense(self):
        ...
        object_container_name_to_id = {
            node["class_name"]: node["id"] for node in object_containers
        }
        object_surface_name_to_id = {
            node["class_name"]: node["id"] for node in object_surfaces
        }
        ...
        with open(f"{os.getcwd()}/mcts/virtualhome/data/obj_commonsense.json") as f:
            obj_commonsense = json.load(f)
        with open(f"{os.getcwd()}/mcts/virtualhome/data/fur_commonsense.json") as f:
        ...
        for node in objects_for_belief_reasoning:
            if (
                ...
                or node["category"] in self.categories_delete
                or node["class_name"] not in obj_commonsense
            ):
                continue
            id1 = node["id"]
            ...
            for container in obj_pos_dist_list:
                if (
                    container[0] == "INSIDE"
                    and container[1] in object_container_name_to_id.keys()
                ):
                    container_id = (
                        container_ids.index(object_container_name_to_id[container[1]])
                        + 1
                    )
                    init_values_in[container_id] = container[2]
                elif (
                    container[0] == "ON"
                    and container[1] in object_surface_name_to_id.keys()
                ):
                    container_id = surface_ids.index(
                        object_surface_name_to_id[container[1]]
                    )
                    init_values_on[container_id] = container[2]
            init_values_in = init_values_in / (
                np.sum(init_values_in) + np.sum(init_values_on)
            )
            ...
            self.edge_belief[id1]["INSIDE"] = [
                [None] + container_ids,
                init_values_in / np.sum(init_values_in),
            ]
            # print(self.edge_belief[id1]['INSIDE'])
            self.edge_belief[id1]["ON"] = [
                surface_ids,
                init_values_on / np.sum(init_values_on),
            ]
    ...
    def init_belief(self):
        ...
        for node in objects_for_belief_reasoning:
            ...
            # Objects are inside containers with uniform probability, that is 1 / (1 + num_containers)
            # the 1 corresponds to the object not being inside any container
            init_values = np.ones(len(container_ids) + 1) / (1.0 + len(container_ids))
            ...
            if node["id"] not in self.edge_belief:
                ...
                if node["id"] in node_inside:
                    ...
                    sample_inside = np.random.choice(
                        ...
                        p=edge_belief_inside[1] / np.sum(edge_belief_inside[1]),
                    )
                    # except:
                ...
                if sample_inside is None:
                    ...
                    try:
                        sample_on = np.random.choice(
                            edge_belief_on[0],
                            p=edge_belief_on[1] / np.sum(edge_belief_on[1]),
                        )
                    except:
    ...
    def update_graph_from_gt_graph(self, gt_graph):
        ...
        for edge in self.sampled_graph["edges"]:
            ...
            if (edge["from_id"] == char_node or edge["to_id"] == char_node) and edge[
                "relation_type"
            ] == "CLOSE":
                continue
    ...
    def update_from_gt_graph(self, gt_graph):
        ...
        for id_node in self.edge_belief.keys():
            ...
            for id_node in self.container_ids:
                if (
                    id_node in visible_ids
                    and id_node in self.container_ids
                    and "OPEN" in id2node[id_node]["states"]
                ):
