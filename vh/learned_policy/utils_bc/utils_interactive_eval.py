...
with open("vh/data_gene/gen_data/data/object_info.json") as f:
...
def get_interactive_input(
    ...
    (
        input_obs_node,
        input_obs_node_mask,
        input_obs_node_state,
        input_obs_node_state_mask,
        input_obs_node_coords,
        input_obs_node_coords_mask,
    ) = get_observation_input(args, data_info, input_obs, agent_id)

    ## history
    history_action_index, history_action_index_mask = get_history_action_input(
        args, data_info, agent_id, all_agent_action, step_i, tokenizer
    )

    ## get goal
    init_unity_graph = deepcopy(vh_envs.init_graph)
    goal_index, goal_index_mask = get_goal_input(
        args, data_info, agent_id, [vh_envs.task_goal], init_unity_graph, tokenizer
    )
...
def check_logical_before_unity(
    agent_id, cur_action, actions_sofar, observations_sofar, logging, verbose=False
):
    ...
    if cur_action is not None:
        ...
        if len(observations_sofar) != 0:
            ...
            interacted_object_in_observation = [
                node
                for node in current_observation[agent_id]["nodes"]
                if node["id"] == interacted_object_id
            ]

            if len(interacted_object_in_observation) == 0:
                if verbose:
                    logging.info(
                        "model error: %s is not in observation" % interacted_object
                    )
                bad_action_flag = True

            if "[putback]" in cur_action or "[putin]" in cur_action:
                interacted_target_in_observation = [
                    node
                    for node in current_observation[agent_id]["nodes"]
                    if node["id"] == target_object_id
                ]

                if len(interacted_target_in_observation) == 0:
                    if verbose:
                        logging.info(
                            "model error: %s is not in observation" % target_object
                        )
                    bad_action_flag = True

        if "[putback]" in cur_action or "[putin]" in cur_action:
            agent_target_close_edge = [
                edge
                for edge in current_observation[agent_id]["edges"]
                if (
                    edge["from_id"] == agent_id + 1
                    and edge["to_id"] == target_object_id
                    and edge["relation_type"] == "CLOSE"
                )
                or (
                    edge["from_id"] == target_object_id
                    and edge["to_id"] == agent_id + 1
                    and edge["relation_type"] == "CLOSE"
                )
            ]
            ...
            if verbose:
                logging.info(
                    "agent_target_close_edge: %s" % str(agent_target_close_edge)
                )

            if len(agent_target_close_edge) == 0:
                if verbose:
                    logging.info(
                        "model error: walk close to %s before putback/putin %s"
                        % (target_object, interacted_object)
                    )
                bad_action_flag = True
            ...
            if "[putback]" in cur_action:
                if target_object not in objects_surface:
                    if verbose:
                        logging.info(
                            "model error: target %s is not a container"
                            % (target_object)
                        )
                    bad_action_flag = True
            if "[putin]" in cur_action:
                ...
                if len(observations_sofar) != 0:
                    ...
                    interacted_target_in_observation = [
                        node
                        for node in current_observation[agent_id]["nodes"]
                        if (node["id"] == target_object_id and "OPEN" in node["states"])
                    ]
                    if len(interacted_target_in_observation) == 0:
                        if verbose:
            ...
            if interacted_object not in objects_grab:
                ...
                if len(observations_sofar) != 0:
                    ...
                    agent_edge = [
                        edge
                        for edge in current_observation[agent_id]["edges"]
                        if edge["from_id"] == agent_id + 1
                        or edge["to_id"] == agent_id + 1
                    ]
                    agent_obj_edge = [
                        edge
                        for edge in agent_edge
                        if edge["from_id"] == interacted_object_id
                        or edge["to_id"] == interacted_object_id
                    ]
                    agent_obj_close_edge = [
                        edge
                        for edge in agent_obj_edge
                    ...
                    if verbose:
                        logging.info(
                            "agent_obj_hold_edge: %s" % str(agent_obj_hold_edge)
                        )
                        logging.info(
                            "agent_obj_close_edge: %s" % str(agent_obj_close_edge)
                        )

                    if len(agent_obj_hold_edge) > 0:
                        if verbose:
                            logging.info(
                                "model error: grab %s that is already in hand"
                                % interacted_object
                            )
                        bad_action_flag = True
                    ...
                    if len(agent_obj_close_edge) == 0:
                        if verbose:
                            logging.info(
                                "model error: %s is grabable, but no close edge"
                                % interacted_object
                            )
                        bad_action_flag = True
        ...
        ## objects cannot be open
            ...
            if len(observations_sofar) != 0:
                ...
                interacted_object_in_observation_open = [
                    node
                    for node in interacted_object_in_observation
                    if "OPEN" in node["states"]
                ]
                if len(interacted_object_in_observation_open) > 0:
                    if verbose:
                        logging.info(
                            "model error: %s is already open" % interacted_object
                        )
                    bad_action_flag = True
            ...
            if len(observations_sofar) != 0:
                ...
                if len(interacted_object_in_observation_open) > 0:
                    if verbose:
                        logging.info(
                            "model error: %s is already closed" % interacted_object
                        )
                    bad_action_flag = True
        ...
        ## grab before put
            ...
            for past_actions in actions_sofar:
                if past_actions[agent_id] is not None:
                    if (
                        "grab" in past_actions[agent_id]
                        and str(interacted_object_id) in past_actions[agent_id]
                    ):
                        check_grab_obj = True
            ...
            if not check_grab_obj:
                if verbose:
                    logging.info(
                        "model error: put before grab %s (%d)"
                        % (interacted_object, interacted_object_id)
                    )
                bad_action_flag = True
            ...
            agent_edge = [
                ...
                if (
                    edge["from_id"] == agent_id + 1
                    and edge["to_id"] == interacted_object_id
                    and edge["relation_type"] == "CLOSE"
                )
                or (
                    edge["to_id"] == agent_id + 1
                    and edge["from_id"] == interacted_object_id
                    and edge["relation_type"] == "CLOSE"
                )
            ]
            if len(agent_edge) > 0:
    ...
    for agent_action in [
        ...
        if agent_action == "walk":
            room_nodes = [
                ...
                if node["class_name"]
                in ["kitchen", "livingroom", "bathroom", "bedroom"]
            ]
            # if len(room_nodes)!=4:
            #     pdb.set_trace()
            ...
            ignore_objs_idx = [
                idx
                for idx, node in enumerate(obs[agent_id]["nodes"])
                if node["class_name"] in ignore_objs
            ]
            # interacted_object_idxs = [tem for tem in list(range(len(obs[agent_id]['nodes']))) if tem not in ignore_objs_idx]
            interacted_object_idxs = [
                (node["id"], node["class_name"])
                for idx, node in enumerate(obs[agent_id]["nodes"])
                if idx not in ignore_objs_idx
            ]

        elif agent_action == "grab":
            agent_edge = [
                edge
                for edge in obs[agent_id]["edges"]
                if edge["from_id"] == agent_id + 1 or edge["to_id"] == agent_id + 1
            ]
            agent_obj_hold_edge = [
                edge for edge in agent_edge if "HOLD" in edge["relation_type"]
            ]
            ...
            grabbable_object_ids = [
                node["id"]
                for node in obs[agent_id]["nodes"]
                if node["class_name"] in objects_grab
            ]
            grabbable_object_ids = [
                tem for tem in grabbable_object_ids if tem not in ignore_objs_id
            ]
            agent_obj_edge = [
                edge
                for edge in agent_edge
                if edge["from_id"] in grabbable_object_ids
                or edge["to_id"] in grabbable_object_ids
            ]
            agent_obj_close_edge = [
                edge for edge in agent_obj_edge if edge["relation_type"] == "CLOSE"
            ]

            if len(agent_obj_close_edge) > 0:
                interacted_object_ids = []
                interacted_object_ids += [
                    edge["from_id"] for edge in agent_obj_close_edge
                ]
                interacted_object_ids += [
                    edge["to_id"] for edge in agent_obj_close_edge
                ]
                ...
                interacted_object_idxs = [
                    (node["id"], node["class_name"])
                    for idx, node in enumerate(obs[agent_id]["nodes"])
                    if node["id"] in interacted_object_ids
                ]
            else:
                continue
            ...
            container_object_nodes = [
                node
                for node in obs[agent_id]["nodes"]
                if node["class_name"] in objects_inside
            ]
            ...
            agent_obj_edge = [
                edge
                for edge in agent_edge
                if edge["from_id"] in container_object_ids
                or edge["to_id"] in container_object_ids
            ]
            agent_obj_close_edge = [
                edge for edge in agent_obj_edge if edge["relation_type"] == "CLOSE"
            ]
            ...
            container_object_nodes = [
                node for node in container_object_nodes if "OPEN" in node["states"]
            ]  ## contrainer is closed
            container_object_ids = [node["id"] for node in container_object_nodes]
            ...
            container_object_nodes = [
                node
                for node in obs[agent_id]["nodes"]
                if node["class_name"] in objects_switchonoff
            ]
            container_object_nodes = [
                node for node in container_object_nodes if ("OFF" in node["states"])
            ]  ## contrainer is closed
            container_object_ids = [node["id"] for node in container_object_nodes]
            ...
            if len(agent_obj_hold_edge) == 0:
                ...
                ignore_tar = [
                    tem[1] for tem in ignore_objs_tars if tem[0] == holding_obj_name
                ]
                ...
                if agent_action == "putin":
                    ...
                    container_object_nodes = [
                        node
                        for node in container_object_nodes
                        if node["class_name"] not in ignore_tar
                    ]
                    container_object_nodes = [
                        node
                        for node in container_object_nodes
                    ...
                    container_object_ids = [
                        node["id"] for node in container_object_nodes
                    ]
                elif agent_action == "putback":
                    container_object_nodes = [
                        node
                        for node in obs[agent_id]["nodes"]
                        if node["class_name"] in objects_surface
                    ]
                    container_object_nodes = [
                        node
                        for node in container_object_nodes
...
def get_valid_action_space(args, agent_action_idx, obs, agent_id):
    ...
    if agent_action == "walk":
        room_nodes = [
            node
            for node in obs[agent_id]["nodes"]
            if node["class_name"] in ["kitchen", "livingroom", "bathroom", "bedroom"]
        ]
        # if len(room_nodes)!=4:
        #     pdb.set_trace()
        ...
        interacted_object_idxs = [
            tem
            for tem in list(range(len(obs[agent_id]["nodes"])))
            if tem not in ignore_objs_idx
        ]
        ...
        container_object_nodes = [
            node for node in container_object_nodes if "CLOSED" in node["states"]
        ]  ## contrainer is closed
        container_object_ids = [node["id"] for node in container_object_nodes]
    ...
    if len(interacted_object_idxs) == 0:
        ...
        if (
            agent_action == "grab"
            or agent_action == "close"
            or agent_action == "switchon"
            or agent_action == "putin"
            or agent_action == "putback"
        ):
            interacted_object_idxs = [
                tem
                for tem in interacted_object_idxs
                if obs[0]["nodes"][tem]["class_name"]
                in args.data_info["history_action_token"]
            ]
            if len(interacted_object_idxs) == 0:
                return None, None
            else:
...
def can_perform_action(
    ...
    if action == "switchon":
        ...
        if graph_helper is not None:
            if (
                id2node[o1_id]["class_name"]
                not in graph_helper.object_dict_types["objects_switchonoff"]
            ):
                return None, True
